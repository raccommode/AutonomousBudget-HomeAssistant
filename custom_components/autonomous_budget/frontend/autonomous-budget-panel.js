import { BudgetLiveElement, baseCSS, dateLabel, esc, icon, labels, money, options } from "./shared.js?v=0.1.0";
...
class AutonomousBudgetPanel extends BudgetLiveElement {
  constructor() {
    ...
    this.shadowRoot.addEventListener("change", (event) => { if (event.target.name === "currency" && this.dialogKind === "item") this.updateRate(); });
    this.render();
  }
  ...
  render() {
    ...
    view.innerHTML = `<div class="shell">
      ...
      <div class="footer row between"><span>Local to your home. In sync with your life.</span><span>Autonomous Budget · 0.1.0</span></div>
    </div>`;
  }
  renderBudgets(budget) {
    return `<nav class="budgets" aria-label="Budgets">${this.data.budgets.map((item) => `<button class="budget-tab ${item.id === budget.id ? "active" : ""}" data-action="select" data-id="${item.id}" aria-current="${item.id === budget.id ? "page" : "false"}">${icon("wallet")}${esc(item.name)}<small>${item.currency}</small></button>`).join("")}</nav>`;
  }
  renderBudget(budget) {
    ...
    return `<div class="period-bar row between"><div><div class="row wrap"><span class="period-title">${dateLabel(budget.period_start, true)} – ${dateLabel(budget.period_last_day, true)}</span><span class="badge">${labels[this.data.settings.period]}</span></div><div class="muted small">${this.offset === 0 ? "Current period" : this.offset < 0 ? "Past period projection" : "Upcoming period projection"} · Scheduled cash flow</div></div><div class="period-nav"><button class="quiet prev" data-action="previous" aria-label="Previous period" ${this.offset <= -120 ? "disabled" : ""}>${icon("chevron")}</button><button class="quiet small" data-action="today">Today</button><button class="quiet" data-action="next" aria-label="Next period" ${this.offset >= 120 ? "disabled" : ""}>${icon("chevron")}</button></div></div>
    <section class="stats" aria-label="Period totals">
      ...
      <div class="stat"><div class="row between"><span class="stat-label">Expected income</span><span class="stat-icon">${icon("down")}</span></div><div class="value number">${m(totals.income)}</div><span class="muted small">${budget.schedule.filter((row) => row.direction === "income").length} scheduled payments this period</span></div>
      <div class="stat"><div class="row between"><span class="stat-label">Planned expenses</span><span class="stat-icon">${icon("arrow")}</span></div><div class="value number">${m(totals.expenses)}</div><span class="muted small">Across your three spending categories</span></div>
    </section>
    <div class="layout"><section class="box"><div class="box-head row between"><div><h2>${esc(budget.name)}</h2><p class="muted small">All your income and commitments</p></div><div class="toolbar"><button class="quiet icon" data-action="dashboard" aria-label="Add budget to a dashboard" title="Dashboard card">${icon("grid")}</button>${this.admin ? `<button class="quiet icon" data-action="edit-budget" aria-label="Edit budget" title="Edit budget">${icon("settings")}</button><button class="primary" data-action="new-item">${icon("plus")}Add entry</button>` : ""}</div></div>
    ...
    ${items.length ? `<div class="table-wrap"><table><thead><tr><th>Entry</th><th>Amount</th><th class="hide-small">Next due</th><th class="hide-medium">This period</th><th><span class="muted">${this.admin ? "Edit" : ""}</span></th></tr></thead><tbody>${items.map((item) => `<tr class="${item.active ? "" : "paused"}"><td><div class="row"><span class="entry-icon ${item.direction}">${item.direction === "income" ? icon("down") : esc(item.name.slice(0, 1).toUpperCase())}</span><div class="entry-name">${esc(item.name)}<div class="entry-meta"><span class="category-dot ${item.category}"></span>${labels[item.category]}${item.active ? "" : " · Paused"}</div></div></div></td><td><div class="amount number ${item.direction === "income" ? "positive" : ""}">${item.direction === "income" ? "+ " : ""}${money(item.amount, item.currency)}</div><div class="small muted">${labels[item.recurrence]}${item.currency !== budget.currency ? ` · × ${esc(item.exchange_rate)}` : ""}</div></td><td class="hide-small"><span>${dateLabel(item.next_due, true)}</span></td><td class="hide-medium"><div class="amount number">${m(item.period_amount)}</div><span class="muted small">${item.occurrences} due</span></td><td>${this.admin ? `<button class="quiet icon" data-action="edit-item" data-id="${item.id}" aria-label="Edit ${esc(item.name)}">${icon("edit")}</button>` : ""}</td></tr>`).join("")}</tbody></table></div>` : `<div class="empty">${icon("calendar")}<h3>${budget.items.length ? "No entries in this view" : "Give your money a plan"}</h3><p class="muted">${budget.items.length ? "Try another filter or add a new entry." : "Start with a paycheck, a subscription, or a bill. Every entry has its own schedule."}</p>${this.admin ? '<button data-action="new-item">Add an entry</button>' : ""}</div>`}
    <div class="table-foot">${budget.items.filter((item) => item.active).length} active entries · Totals count payments due in the selected period.</div></section>
    <aside class="side"><section class="box"><div class="box-head"><h2>Where it goes</h2><p class="muted small">Planned expenses by category</p></div><div class="category-list">${["investment", "mandatory", "optional"].map((category) => { const pct = Number(totals.expenses) ? Number(totals[category]) / Number(totals.expenses) * 100 : 0; return `<div><div class="row"><span class="category-icon ${category}">${icon(category)}</span><div class="spacer"><h3>${labels[category]}</h3><span class="muted small">${Math.round(pct)}% of expenses</span></div><span class="amount number">${m(totals[category])}</span></div><div class="bar"><span class="${category}" style="width:${pct}%"></span></div></div>`; }).join("")}</div></section>
    <section class="box"><div class="box-head row between"><h2>Coming up</h2>${icon("calendar")}</div><div class="schedule">${upcoming.length ? upcoming.map((item) => `<div class="due-row row"><div class="date-tile"><span>${new Intl.DateTimeFormat("en", {month:"short"}).format(new Date(`${item.date}T12:00:00`))}</span><b>${Number(item.date.slice(-2))}</b></div><div class="spacer due-name">${esc(item.name)}<div class="muted small">${labels[item.direction]}</div></div><span class="amount number ${item.direction === "income" ? "positive" : ""}">${item.direction === "income" ? "+ " : ""}${m(item.amount)}</span></div>`).join("") : '<p class="muted small" style="padding:0 0 18px">Nothing else is due in this period.</p>'}</div></section><p class="note">Your plan follows actual due dates. Monthly bills appear in the pay period when they renew. These are projections, not cleared bank transactions.</p></aside></div>`;
  }
  async onClick(event) {
    ...
    if (["previous", "next", "today"].includes(action)) { this.offset = action === "today" ? 0 : this.offset + (action === "next" ? 1 : -1); this.subscribe(); return; }
  ...
  openSettings() {
    ...
    this.openDialog("Budget settings", `<div class="dialog-content" style="padding-bottom:0"><p class="help">All budgets follow this schedule. For a two-week budget, set the reference date to a payday. The currency below is the default for new budgets.</p></div>${this.form(this.selectField("Default currency", "currency", Object.keys(this.data.currencies).sort(), settings.currency) + this.selectField("Budget period", "period", ["daily", "weekly", "biweekly", "monthly", "yearly"], settings.period) + this.field("Reference date / payday", "anchor", settings.anchor, "date", true, 'required min="1900-01-01" max="2200-12-31"'))}`, "settings");
  }
  openBudget(edit) {
    this.editing = edit ? this.budget : null;
    this.openDialog(edit ? "Edit budget" : "Create a budget", this.form(this.field("Budget name", "name", edit ? this.budget.name : "", "text", true, 'required maxlength="100" placeholder="e.g. Everyday life"') + this.selectField("Currency", "currency", Object.keys(this.data.currencies).sort(), edit ? this.budget.currency : this.data.settings.currency, edit && this.budget.items.length ? "disabled" : "") + '<p class="muted small">The period follows your global budget settings.</p>', edit ? "Save changes" : "Create budget", edit ? `<button type="button" class="quiet danger icon" data-action="delete-budget" aria-label="Delete budget">${icon("trash")}</button>` : ""), "budget");
  }
  openItem(item) {
    this.editing = item;
    const entry = item || { name: "", direction: "expense", category: "optional", amount: "", currency: this.budget.currency, exchange_rate: "1", recurrence: "monthly", renewal_date: this.data.today, end_date: "", active: true };
    const fields = this.field("Entry name", "name", entry.name, "text", true, 'required maxlength="100" placeholder="e.g. Netflix, rent, or paycheck"') + this.selectField("Money flow", "direction", ["income", "expense"], entry.direction) + this.selectField("Category", "category", ["investment", "mandatory", "optional"], entry.category) + this.field("Amount", "amount", entry.amount, "number", false, 'required min="0" max="1000000000" step="any" inputmode="decimal"') + this.selectField("Currency", "currency", Object.keys(this.data.currencies).sort(), entry.currency) + `<label class="field full fx">Exchange rate to ${this.budget.currency}<input type="number" name="exchange_rate" value="${esc(entry.exchange_rate)}" min="0.00000001" max="1000000" step="any" required/><small>1 unit of this entry's currency equals this many ${this.budget.currency}. Rates are set manually.</small></label>` + this.selectField("Repeats", "recurrence", ["once", "daily", "weekly", "biweekly", "monthly", "quarterly", "yearly"], entry.recurrence) + this.field("First due / renewal date", "renewal_date", entry.renewal_date, "date", false, 'required min="1900-01-01" max="2200-12-31"') + this.field("End date (optional, inclusive)", "end_date", entry.end_date, "date", true, 'min="1900-01-01" max="2200-12-31"') + `<label class="checkbox"><input type="checkbox" name="active" ${entry.active ? "checked" : ""}/>Active entry</label><span class="muted small">Pause to exclude all occurrences.</span>`;
  ...
  confirmDelete(kind, item) {
    this.editing = item;
    this.openDialog(`Delete ${kind}?`, `<form><p>Delete <strong>${esc(item.name)}</strong>${kind === "budget" ? " and all its entries" : ""}? This cannot be undone.</p><div class="form-error" role="alert"></div><div class="dialog-actions"><button type="button" data-action="close">Cancel</button><button type="submit" class="danger">Delete ${kind}</button></div></form>`, `delete-${kind}`);
  }
  openDashboard() {
    const yaml = `type: custom:autonomous-budget-card\nbudget_id: ${this.budget.id}\nshow_categories: true\nshow_upcoming: true`;
    this.openDialog("Your budget, on any dashboard", `<div class="dialog-content"><p class="muted">The card is already registered with Home Assistant.</p><h3>Use the visual editor</h3><p>Edit a dashboard → Add card → search for <strong>Autonomous Budget</strong>, then select this budget.</p><h3>Or paste this YAML</h3><pre>${esc(yaml)}</pre><p class="muted small">Six native sensors are also available under Settings → Devices & services → Autonomous Budget.</p><div class="dialog-actions"><button data-action="close" class="primary">Done</button></div></div>`, "dashboard");
  }
  ...
  exportData() {
    const data = { format: "autonomous_budget", version: 1, exported_at: new Date().toISOString(), settings: this.data.settings, budgets: this.data.budgets.map(({ id, name, currency, items }) => ({ id, name, currency, items: items.map(({ period_amount, occurrences, next_due, ...item }) => item) })) };
