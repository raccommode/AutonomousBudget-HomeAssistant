...
class AutonomousBudgetPanel extends BudgetLiveElement {
  constructor() {
    ...
    this.filter = "all";
    this.viewMode = "plan";
    this.shadowRoot.innerHTML = `<style>${baseCSS}${css}${workspaceCSS}</style><main class="view"></main><dialog class="dialog" aria-labelledby="dialog-title"></dialog><div role="status" aria-live="polite" class="notifications"></div>`;
  ...
  get canEdit() { return !!this.hass?.user; }
  get canCreate() { return this.hass?.user?.is_admin; }
  render() {
    ...
    view.innerHTML = `<div class="shell">
      ...
      <div class="footer row between"><span>Stored in Home Assistant</span><span>Autonomous Budget · 1.5.1</span></div>
    </div>`;
  }
  ...
  renderSharing(budget) {
    ...
    return `<section class="box sharing" aria-label="Shared budget allocation"><div class="box-head row between"><div><h2>Shared budget</h2><p class="muted small">Automatic contributions to personal budgets</p></div>${this.canEdit ? '<button data-action="allocation">Manage allocation</button>' : ""}</div>
      <div class="sharing-list">${sharing.members.length ? sharing.members.map((member) => `<div class="share-row"><div class="share-person"><button class="quiet" data-action="select" data-id="${member.budget_id}"><span translate="no">${esc(member.name)}</span></button><div class="muted small">${esc(member.percentage)}% · ${labels[member.period]}</div></div><div class="share-amount"><strong class="number">${this.money(member.amount, member.currency || budget.currency)}</strong><div class="muted small">Next contribution · ${this.dateLabel(member.next_due, true)}</div></div></div>`).join("") : '<p class="muted small">Choose the personal budgets and percentages to share these expenses.</p>'}</div>
  ...
  renderEntries(budget, items, planned, totals) {
    ...
    return `<div class="table-wrap"><table><thead><tr><th>Entry</th><th>${planned ? "Per pay period" : "Amount"}</th><th class="hide-small">Next due</th><th class="hide-medium">${planned ? "Original amount" : "This period"}</th><th><span class="muted">${this.canEdit ? "Edit" : ""}</span></th></tr></thead>
  ...
  renderReserves(budget) {
    ...
    return `<section class="box reserves" aria-label="Projected reserves"><div class="box-head row between wrap"><div><h2>Projected reserves</h2><p class="muted small">Today · ${this.dateLabel(budget.reserves.as_of, true)}</p></div><div class="row"><strong class="number reserve-total">${m(budget.reserves.amount)}</strong><button class="quiet icon" data-action="reserve-entity" aria-label="Home Assistant entity" title="Home Assistant entity">${icon("grid")}</button></div></div>
      ...
      <div class="available-summary"><div class="row between wrap"><h3>Available after reserves</h3><strong class="number ${Number(budget.available_balance) < 0 ? "negative" : "positive"}">${budget.available_balance === null ? "—" : m(budget.available_balance)}</strong></div><p class="muted small">${budget.available_balance === null ? (budget.conversion_missing ? "Missing exchange rate for linked accounts." : "Add an account balance in Edit budget to see this estimate.") : `<span>Account balance</span> ${m(budget.account_balance)} − <span>Credit owed</span> ${m(budget.credit_balance || "0")} + <span>Projected reserves</span> (${m(budget.reserves.amount)})`}</p><p class="muted small">Estimated amounts, assuming earlier installments were saved and due bills were paid. ${budget.linked_accounts ? "Balances come from linked accounts." : "Update manual balances as needed."}</p></div></section>`;
  }
  async onClick(event) {
    ...
    if (!this.canEdit || (action === "new-budget" && !this.canCreate)) return;
  ...
  openAllocation() {
    ...
    const eligible = this.data.budgets.filter((budget) => budget.kind !== "shared");
    const allocations = new Map(this.budget.allocations.map((row) => [row.budget_id, row.percentage]));
    const fields = `<p class="muted small full" style="grid-column:1/-1">Create a personal budget for each person. For another currency, enter a planning exchange rate. Each person’s pay schedule determines contribution dates.</p>` + eligible.map((budget) => `<div class="allocation-row"><div><span translate="no">${esc(budget.name)}</span><div class="muted small">${labels[budget.effective_period]} · ${this.dateLabel(budget.effective_anchor, true)}</div></div>${this.field("Share (%)", `share_${budget.id}`, allocations.get(budget.id) || "0", "number", false, `required min="0" max="100" step="0.01" data-budget-id="${budget.id}"`)}${budget.currency !== this.budget.currency ? this.field("Planning exchange rate", `fx_${budget.id}`, this.budget.allocations.find(a=>a.budget_id===budget.id)?.exchange_rate || "", "number", false, 'min="0.00000001" step="any"') + `<span translate="no">${esc(this.budget.currency)} → ${esc(budget.currency)}</span>` : ""}</div>`).join("") + '<p class="muted small allocation-total" aria-live="polite"></p><p class="muted small" style="grid-column:1/-1">Set a share to 0 to remove the contribution. Unallocated amounts remain in the shared budget.</p>';
  ...
  async openBudget(edit) {
    ...
    if (edit && this.budget.linked_accounts) {
      ...
      form.elements.credit_balance.disabled=true;
      const note=form.querySelector('[name="credit_balance"]').closest("label").nextElementSibling;
      if(note) note.textContent=this.t("Balances come from linked accounts. Manage allocations in Finance settings.");
    }
  }
  openItem(item) {
  ...
  openReserveEntity() {
    ...
    if (!entity && this.budget.linked_accounts) {
      this.openDialog("Reserve entity",`<div class="dialog-content"><p>Native sensors are disabled for linked accounts until their amounts are explicitly published in Edit account.</p><button data-action="close">Done</button></div>`,"reserve-entity");
      return;
    }
    if (!entity) {
  ...
  async onSubmit(event) {
    ...
    else if (this.dialogKind === "allocation") { action = "budget_update"; payload = { budget_id: this.editing.id, allocations: [...form.querySelectorAll("input[data-budget-id]")].filter((input) => Number(input.value) > 0).map((input) => ({budget_id: input.dataset.budgetId, percentage: input.value, ...(form.elements[`fx_${input.dataset.budgetId}`] ? {exchange_rate: form.elements[`fx_${input.dataset.budgetId}`].value} : {})})) }; }
...
if (!customElements.get("autonomous-budget-view")) customElements.define("autonomous-budget-view", AutonomousBudgetPanel);
