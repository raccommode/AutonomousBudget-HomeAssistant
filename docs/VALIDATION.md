...
- Version 1.5.1 adds status-specific 400/422 diagnostics, bounded JSON error reading, credential redaction, malformed/oversized response handling, preview/save/recovery and escaped provider explanations in English and French. A live HTTP 400 was observed for a Wise account, but the installed version discarded its JSON body; the underlying rejection is not yet diagnosed or claimed fixed.
- Version 1.5.0 adds repeat-within-day scheduling, failure isolation, cadence changes, opt-out, cancellation and overlapping-tick checks; independent conversion-rate fetching and backoff; mixed-currency cash/stock/crypto totals without duplicate local positions; missing/zero amounts and backup-preserved conversion settings. Two additional browser journeys verify localized currency selectors, converted real Home Assistant states, disabling conversion and refresh settings on English desktop and French mobile.
- 237 Python tests: existing budget calculations and identifiers; negative and income-day reserves; SQLite migration/rollback; household access and administrator creation guards; exact splits, transfers, refunds, reconciliation; dated exchange rates; average/FIFO positions and cost-preserving security transfers; loan projections/payments; imports and restoration; budget allocations; provider failures and synchronization conflicts.
- Account-sensor regression tests reproduce a zero journal balance with a CAD 524 bank snapshot and assert real Home Assistant entity states. They cover automatic refresh before initial journal import, retained values during provider failures, zero/negative balances, unlinking, missing snapshots, manual accounts and explicit publication. Provider responses use fictional fixtures; this does not constitute verification on a personal Home Assistant installation.
...
Version 1.5.0 UI previews: [currency conversion on French mobile](screenshot-currency-fr-mobile.png), [currency conversion on English desktop](screenshot-currency-en-desktop.png), [refresh settings on French mobile](screenshot-refresh-fr-mobile.png), and [refresh settings on English desktop](screenshot-refresh-en-desktop.png).

Version 1.5.1 UI previews: [provider explanation on French mobile](screenshot-provider-error-fr.png) and [English desktop](screenshot-provider-error-en.png).

## External provider verification

On September 10, a read-only inspection of the existing account pages showed several provider-side bank-balance errors. A user-authorized account refresh left one affected account unavailable. The new diagnostics and retry schedule are tested with representative HTTP responses; they cannot guarantee recovery when Lunch Flow or the bank continues to withhold a balance. No production deployment of 1.5.1 is claimed here.
