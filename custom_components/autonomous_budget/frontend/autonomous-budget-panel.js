import { BudgetLiveElement, baseCSS, esc, icon, labels, options } from "./shared.js?v=0.1.0";
...
class AutonomousBudgetPanel extends BudgetLiveElement {
  ...
  renderBudgets(budget) {
    return `<nav class="budgets" aria-label="Budgets">${this.data.budgets.map((item) => `<button class="budget-tab ${item.id === budget.id ? "active" : ""}" data-action="select" data-id="${item.id}" aria-current="${item.id === budget.id ? "page" : "false"}">${icon("wallet")}<span translate="no">${esc(item.name)}</span><small>${item.currency}</small></button>`).join("")}</nav>`;
  }
  renderBudget(budget) {
    ...
    return `<div class="period-bar row between"><div><div class="row wrap"><span class="period-title">${this.dateLabel(budget.period_start, true)} – ${this.dateLabel(budget.period_last_day, true)}</span><span class="badge">${labels[budget.effective_period]}</span></div><div class="muted small">${this.offset === 0 ? "Current period" : this.offset < 0 ? "Past period projection" : "Upcoming period projection"} · Scheduled cash flow</div></div><div class="period-nav"><button class="quiet prev" data-action="previous" aria-label="Previous period" ${this.offset <= -120 ? "disabled" : ""}>${icon("chevron")}</button><button class="quiet small" data-action="today">Today</button><button class="quiet" data-action="next" aria-label="Next period" ${this.offset >= 120 ? "disabled" : ""}>${icon("chevron")}</button></div></div>
    <section class="stats" aria-label="Period totals">
      ...
      <div class="stat"><div class="row between"><span class="stat-label">Planned expenses</span><span class="stat-icon">${icon("arrow")}</span></div><div class="value number">${m(totals.expenses)}</div><span class="muted small">All payments due this period</span></div>
    </section>
    <div class="layout"><section class="box"><div class="box-head row between"><div><h2><span translate="no">${esc(budget.name)}</span></h2><p class="muted small">All your income and commitments</p></div><div class="toolbar"><button class="quiet icon" data-action="dashboard" aria-label="Add budget to a dashboard" title="Dashboard card">${icon("grid")}</button>${this.admin ? `<button class="quiet icon" data-action="edit-budget" aria-label="Edit budget" title="Edit budget">${icon("settings")}</button><button class="primary" data-action="new-item">${icon("plus")}Add entry</button>` : ""}</div></div>
    ...
    <aside class="side"><section class="box"><div class="box-head"><h2>Where it comes from</h2><p class="muted small">Expected income by category</p></div><div class="category-list">${["investment", "mandatory", "optional"].map((category) => { const pct = Number(totals.income) ? Number(totals[category]) / Number(totals.income) * 100 : 0; return `<div><div class="row"><span class="category-icon ${category}">${icon(category)}</span><div class="spacer"><h3>${labels[category]}</h3><span class="muted small">${Math.round(pct)}% of income</span></div><span class="amount number">${m(totals[category])}</span></div><div class="bar"><span class="${category}" style="width:${pct}%"></span></div></div>`; }).join("")}</div></section>
  ...
  updateCategory() {
    ...
    const income = form.elements.direction.value === "income";
    form.elements.category.disabled = !income;
    form.elements.category.closest("label").style.display = income ? "flex" : "none";
    form.elements.direction.closest("label").style.gridColumn = income ? "" : "1/-1";
  }
  updateRate() {
    const form = this.shadowRoot.querySelector("dialog form");
  ...
  exportData() {
    const data = { format: "autonomous_budget", version: 1, exported_at: new Date().toISOString(), settings: this.data.settings, budgets: this.data.budgets.map(({ id, name, currency, period, anchor, items }) => ({ id, name, currency, period: period || null, anchor: anchor || null, items: items.map(({ period_amount, occurrences, next_due, ...item }) => item) })) };
