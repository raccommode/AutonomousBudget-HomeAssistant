import { BudgetLiveElement, baseCSS, esc, icon, labels, options } from "./shared.js?v=1.0.2";
...
class AutonomousBudgetPanel extends BudgetLiveElement {
  ...
  render() {
    ...
    view.innerHTML = `<div class="shell">
      ...
      <div class="footer row between"><span>Local to your home. In sync with your life.</span><span>Autonomous Budget · 1.0.2</span></div>
    </div>`;
  }
  ...
  openReserveEntity() {
    ...
    if (!entity && this.budget.linked_accounts) {
      this.openDialog("Reserve entity",`<div class="dialog-content"><p>Native sensors are withheld for private linked accounts. The account owner must explicitly publish their amounts in Finance settings.</p><button data-action="close">Done</button></div>`,"reserve-entity");
      return;
    }
    if (!entity) {
