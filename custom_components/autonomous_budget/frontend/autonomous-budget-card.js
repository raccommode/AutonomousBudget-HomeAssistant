import { BudgetLiveElement, baseCSS, esc, icon, labels } from "./shared.js?v=0.1.0";
...
class AutonomousBudgetCard extends BudgetLiveElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `<style>${baseCSS}${css}</style><ha-card><header class="row between"><div><h2><span translate="no">${esc(this.config.title || budget.name)}</span></h2><span class="muted">${this.dateLabel(budget.period_start)} – ${this.dateLabel(budget.period_last_day, true)} · ${labels[budget.effective_period]}</span></div><span class="mark">${icon("wallet")}</span></header><div class="muted small">Left after expenses</div><div class="value number ${Number(budget.totals.balance) < 0 ? "negative" : "positive"}">${m(budget.totals.balance)}</div><div class="muted small">Scheduled for this period</div><div class="totals"><div><span class="muted small">Expected income</span><b class="number">${m(budget.totals.income)}</b></div><div><span class="muted small">Planned expenses</span><b class="number">${m(budget.totals.expenses)}</b></div></div>${this.config.show_categories ? `<h3 style="margin-top:17px">Income by category</h3><div class="categories">${["investment", "mandatory", "optional"].map((category) => `<div class="category"><span class="small muted"><i class="category-dot ${category}"></i> ${labels[category]}</span><b class="number">${m(budget.totals[category])}</b></div>`).join("")}</div>` : ""}${this.config.show_upcoming ? `<div class="due"><h3>Coming up</h3>${due.length ? due.map((item) => `<div class="row between due-row"><span><span translate="no">${esc(item.name)}</span> <span class="muted small">· ${this.dateLabel(item.date)}</span></span><span class="number">${item.direction === "income" ? "+ " : ""}${m(item.amount)}</span></div>`).join("") : '<p class="muted small">Nothing else is due this period.</p>'}</div>` : ""}<a href="/autonomous-budget">Open Autonomous Budget ${icon("chevron")}</a></ha-card>`;
  }
}
