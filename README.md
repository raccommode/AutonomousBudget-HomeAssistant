...
An expense is treated as paid directly with a positive, active income in the **same budget** only when both entries have the **same frequency as the budget’s pay period** and their scheduled payment dates coincide. For a biweekly pay period, a monthly, quarterly or annual bill still builds its normal fractional reserve even when a renewal happens to fall on payday. It contributes **zero to projected reserves**. Its amount, expense category, scheduled cash flow, per-pay-period plan, and common-budget amount stay unchanged.
...
The comparison uses actual scheduled dates, including recurrence, month-end adjustments, first due dates, and inclusive end dates. One-time income and income with a different frequency do not qualify. Paused income, zero income, and income from another budget do not qualify. A budget's reference date alone is not an income entry. The rule checks both frequency and date, regardless of whether the income covers all qualifying expenses on that date.
...
Investment account entities add the market values of stocks, funds and cryptocurrencies to cash, counting bank positions once. In Add/Edit account, use the currency dropdown and **Convert automatically → Convert to** to publish this total in your preferred currency. The panel previews the entity value; missing required values or exchange rates produce an unknown total. Lunch Flow refreshes at startup and hourly by default; change its frequency under **Finance settings → Lunch Flow → Automatic refresh**.
