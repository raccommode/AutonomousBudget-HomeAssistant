...
Choose **Account currency** from the dropdown. To display another currency, enable **Convert automatically** and choose **Convert to** in Add/Edit account. The account and journal retain their original currency. Its published Home Assistant entity uses the target currency; account pages show a preview under **Home Assistant entity value**. Turning conversion off restores the account currency without recreating the entity. This converts a displayed value, not funds at the bank.

Investment account entities include cash plus all available position market values, with bank positions replacing local positions rather than being added twice. Each component is converted before the final total is rounded. The main portfolio entity covers that account's cash and securities; separately created currency pockets keep their own account entities. Missing required cash, holdings, prices or exchange rates produce an unknown total. Last valid bank snapshots are retained on transient failures and marked stale. Account conversion fetches required rates when saved and every six hours, retrying failed rate requests after 15 minutes; a manually entered rate for the current day takes precedence. No account amounts are sent to the exchange-rate provider.
...
Enter corrections through **Finance settings → Exchange rate**, fetch a dated rate, or use **Get missing exchange rates** in Reports to retrieve historical rates. A manual rate wins over an automatic rate on the same date. Automatic FX uses [Frankfurter](https://frankfurter.dev/) without an API key; **Refresh exchange rates automatically** is opt-in for reports. Account entity conversion independently enables the rates required by that account. Budget planning exchange rates remain explicit assumptions and are not overwritten by market rates.
...
Enable automatic quotes per instrument for daily refreshes, or refresh manually. Requests use a cache and rate-limit cooldown; the background scheduler checks every minute, skips instruments already refreshed that local day, and retries failed quotes after an hour. Network failures keep the last saved price. Prices show their original dates; no historical market series is invented before the first available quote.
